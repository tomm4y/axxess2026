...
function getSessionId(req: { params: Record<string, unknown> }): string {
...
export function createS2TController(service: S2TService) {
  const speakerMapSchema = z.record(z.string(), z.enum(['Doctor', 'Patient']))
  ...
  return {
    ...
    getSession: ((req, res) => {
      const s = service.getSessionOrThrow(getSessionId(req))
      res.status(200).json(service.publicView(s))
    }) satisfies RequestHandler,

    stopSession: ((req, res) => {
      const s = service.stopSession(getSessionId(req), 'stopped via http')
      res.status(200).json({
    ...
    setSpeakerMap: ((req, res) => {
      ...
      const s = service.setSpeakerMap(getSessionId(req), mapping)
      res.status(200).json({ sessionId: s.sessionId, speakerMap: s.speakerMap })
    }) satisfies RequestHandler,

    autoMapSpeakers: ((req, res) => {
      const s = service.autoMapSpeakers(getSessionId(req))
      res.status(200).json({ sessionId: s.sessionId, speakerMap: s.speakerMap })
    }) satisfies RequestHandler,
    ...
    deleteSession: ((req, res) => {
      service.deleteSession(getSessionId(req))
      res.status(204).send()
    }) satisfies RequestHandler,
  }
}
